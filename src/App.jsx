...
const LINKEDIN_CONFIG = {
  orgId: "YOUR_ORG_ID",
  certName: "Google cloud study jam'25 Certificate",
  orgName: "GDG on Campus BCE Patna (Formerly GDSC)"
};
...
export default function App() {
  ...
  return (
    <div className="relative min-h-[100dvh] w-full bg-[#070b19] font-sans selection:bg-cyan-500/30 text-white flex flex-col overflow-x-hidden">
      
      {/* --- PREMIUM BACKGROUND PATTERN & BLOBS --- */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        {/* Subtle dot pattern for premium tech feel */}
        <div className="absolute inset-0 bg-[radial-gradient(rgba(255,255,255,0.06)_1px,transparent_1px)] [background-size:24px_24px] opacity-60"></div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center relative z-10 w-full p-4 sm:p-8">
        
        {isVerifyRoute ? (
          /* =========================================================================
             VERIFICATION VIEW (When accessed via QR Code / URL Param)
             ========================================================================= */
          <div className="w-full max-w-md backdrop-blur-3xl bg-slate-900/50 border border-white/[0.08] p-8 rounded-[2rem] shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] transition-all duration-500 hover:border-white/[0.15]">
            
            {verifyStatus === "loading" && (
              <div className="flex flex-col items-center text-center space-y-6 animate-in fade-in duration-500">
                <div className="w-20 h-20 bg-blue-500/10 rounded-full flex items-center justify-center border border-blue-500/20 shadow-[0_0_20px_rgba(59,130,246,0.2)]">
                  <Loader2 className="w-10 h-10 text-blue-400 animate-spin" />
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-white mb-2">Verifying Record...</h2>
                  <p className="text-slate-400">Checking the official database for ID: <br/><span className="text-white font-mono mt-1 inline-block bg-white/5 px-2 py-1 rounded">{verifyIdQuery}</span></p>
                </div>
              </div>
            )}
            ...
            {verifyStatus === "success" && verifyData && (
              <div className="flex flex-col items-center text-center space-y-6 animate-in zoom-in-95 duration-500">
                ...
                <div className="w-full">
                  ...
                  <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-6 text-left space-y-4 hover:bg-white/[0.05] transition-colors">
                    <div>
                  ...
                  <button 
                    ...
                    className="mt-8 text-slate-400 hover:text-cyan-400 flex items-center justify-center gap-2 w-full transition-colors font-medium"
                  >
                ...
                <div>
                  ...
                  <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-6 text-left">
                    <p className="text-sm text-slate-300">
                      The requested ID (<span className="font-mono text-white bg-white/5 px-1 py-0.5 rounded">{verifyIdQuery}</span>) does not match any official records in the {LINKEDIN_CONFIG.orgName} database.
                    </p>
                  </div>
          ...
          /* =========================================================================
             ...
             ========================================================================= */
          <div className="w-full max-w-5xl backdrop-blur-3xl bg-white/[0.03] border border-white/[0.08] p-6 sm:p-12 rounded-[2.5rem] shadow-[0_8px_32px_0_rgba(0,0,0,0.4)] transition-all duration-500 hover:shadow-[0_8px_40px_0_rgba(6,182,212,0.1)] hover:border-white/[0.12]">
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              ...
              <div className="relative">
                ...
                <div className="absolute inset-0 bg-gradient-to-tr from-cyan-500/15 to-purple-500/15 rounded-3xl blur-2xl" />
                
                <div className="relative backdrop-blur-2xl bg-slate-900/60 border border-white/[0.1] rounded-3xl p-6 sm:p-8 shadow-2xl min-h-[400px] flex flex-col justify-center transition-all duration-300">
                  {!result ? (
                    /* --- SEARCH FORM --- */
                    <form onSubmit={handleSearch} className="space-y-6">
                      <div className="text-center space-y-2 mb-8">
                        <div className="w-16 h-16 bg-gradient-to-br from-cyan-500/20 to-purple-500/20 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-white/10 shadow-[0_0_20px_rgba(6,182,212,0.2)]">
                      ...
                      <div className="space-y-4">
                        <div>
                          <input
                            ...
                            className="block w-full px-5 py-4 bg-black/20 border border-white/[0.1] rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 focus:bg-black/40 transition-all duration-300"
                    ...
                    <div className="space-y-6 animate-in fade-in zoom-in duration-500">
                      ...
                      <div className="rounded-xl overflow-hidden border border-white/10 shadow-2xl relative group bg-black/50 p-2">
                        <img
                          src={result.url}
                          alt={`${result.name}'s Certificate`}
                          className="w-full object-contain max-h-[350px] rounded-lg"
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-6">
                          <div className="flex items-center gap-2 text-white/80 bg-black/40 px-3 py-1.5 rounded-full backdrop-blur-md">
                            <QrCode className="w-4 h-4" />
                            <span className="text-xs font-medium tracking-wider">Scan to Verify</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                        <a
                          ...
                          className="flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/10 text-white font-medium backdrop-blur-md transition-all duration-300 hover:shadow-lg"
                        >
                        ...
                        <button
                          ...
                          className="flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl bg-[#0a66c2]/80 hover:bg-[#0a66c2] border border-[#0a66c2]/50 text-white font-medium backdrop-blur-md transition-all duration-300 shadow-[0_0_15px_rgba(10,102,194,0.3)] hover:shadow-[0_0_20px_rgba(10,102,194,0.5)]"
                        >
      ...
      {/* =========================================================================
          GLOBAL FOOTER
          ========================================================================= */}
      <footer className="relative z-10 w-full mt-auto border-t border-white/10 bg-black/30 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4 text-center md:text-left">
            ...
            <p className="text-slate-400 text-sm font-medium">
              © {new Date().getFullYear()} {LINKEDIN_CONFIG.orgName}
            </p>
          </div>

          {/* Credits - Made more attractive */}
          <div className="flex items-center gap-1.5 text-slate-400 text-sm font-medium bg-white/5 px-4 py-2 rounded-full border border-white/5 shadow-inner">
            Crafted with <Heart className="w-4 h-4 text-red-500 animate-pulse fill-red-500" /> by
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 font-bold ml-1 hover:opacity-80 transition-opacity cursor-pointer drop-shadow-[0_0_8px_rgba(6,182,212,0.4)]">
              Tech Lead Sunny
            </span>
          </div>
